efmodule Readability.DescriptionFinderTest do
  use ExUnit.Case, async: true

  doctest Readability.DescriptionFinder
  ...
  test "extract most proper title" do
    title = Readability.DescriptionFinder.title(@html_tree)
  ...
  test "extract og title" do
    title = Readability.DescriptionFinder.og_title(@html_tree)
    assert title == "og title"
  end
  ...
  test "does not merge multiple matching og:title tags" do
    ...
    title = html |> Floki.parse_fragment!() |> Readability.DescriptionFinder.og_title()
    assert title == "og title 1"
  end

  test "extract tag title" do
    title = Readability.DescriptionFinder.tag_title(@html_tree)
    ...
    title = html |> Floki.parse_fragment!() |> Readability.DescriptionFinder.tag_title()
    assert title == "Tag title"
  ...
  test "extract h1 tag title" do
    title = Readability.DescriptionFinder.h_tag_title(@html_tree)
    assert title == "h1 title"
  end

  test "extract h2 tag title" do
    title = Readability.DescriptionFinder.h_tag_title(@html_tree, "h2")
  ...
  test "does not merge multile header tags" do
    ...
    title = html |> Floki.parse_fragment!() |> Readability.DescriptionFinder.h_tag_title()
    assert title == "header 1"
  end

  test "returns an empty string when no title tag can be found" do
    assert Readability.DescriptionFinder.tag_title([]) == ""
  end

  test "returns an empty string when no og:title tag can be found" do
    assert Readability.DescriptionFinder.og_title([]) == ""
  end

  test "returns an empty string when no header tag can be found" do
    assert Readability.DescriptionFinder.h_tag_title([]) == ""
  end
end
